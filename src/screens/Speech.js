...
const Iq = ({ navigation }) => {
	...
	const [ currWord, setCurWord ] = useState(words[Math.floor(Math.random() * words.length)]);
	const [ clientID, setClientID ] = useState('bJQNgLuTGTSRJMsTOETfYTstQMWrqLWqGn6pQvSYEkA');
	useEffect(() => {
		// setCurWord(words[Math.floor(Math.random() * words.length)]);
		const url = 'https://api.unsplash.com/search/photos?page=1&query=' + currWord + '&client_id=' + clientID;
	...
	function renderQuiz() {
		return (
			<SafeAreaView style={styles.container}>
				<ImageBackground style={{ flex: 1 }} source={require('../assets/yelback.jpg')}>
					<Modal animationType="slide" transparent={true} visible={modalVisible}>
						<View style={styles.centeredView}>
							<View style={styles.modalView}>
								<Text style={styles.modalText}>Please read the word loud and clear</Text>
								<Image
