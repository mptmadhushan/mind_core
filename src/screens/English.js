		...
		if (theArray.length === 10) {
			...
			getResultAPIEnglish(theArray)
				.then((response) => {
					...
					storeEnglishMarks(resData).then((result) => console.log('Remove me if not needed Iq', result));
	...
	function renderQuiz() {
		return (
			<SafeAreaView style={styles.container}>
				<ImageBackground style={{ flex: 1 }} source={require('../assets/IQBACK.jpeg')}>
								...
								{numbers.map((number, i) => (
									<View
										...
										style={[
											styles.number1,
											{
												backgroundColor: currQuiz !== number ? COLORS.primary : COLORS.fourth
											}
										]}
									>
